...
CONF_WAKEUP_PIN_KEEP_FLOATING = "wakeup_pin_keep_floating"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_WAKEUP_PIN_KEEP_FLOATING): cv.boolean,
        }
    ),
)
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_WAKEUP_PIN_KEEP_FLOATING): cv.All(
            cv.only_on(["libretiny", "bk72xx"]), cv.boolean
        ),
...
async def to_code(config):
    ...
    if CONF_WAKEUP_PIN_KEEP_FLOATING in config:
        cg.add(var.set_wakeup_pin_keep_floating(config[CONF_WAKEUP_PIN_KEEP_FLOATING]))
            ...
            cg.add(
                var.add_wakeup_pin(
                    cg.StructInitializer(
                        ...
                        (
                            "wakeup_pin_keep_floating",
                            item.get(
                                CONF_WAKEUP_PIN_KEEP_FLOATING,
                                False,
                            ),
                        )
                    )
